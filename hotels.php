...
} elseif (!empty($_GET['villePOST'])) {
  $ville = mysqli_real_escape_string($conn, $_GET['villePOST']);
}
...
<div class="container">
  ...
  <form class="filters" method="get">
    ...
    <label>Ville sélectionnée :</label>
    <input type="text" value="<?= htmlspecialchars($ville) ?>" readonly style="background-color: #eee; padding: 6px; border: 1px solid #ccc; width: 100%; border-radius: 6px;">
    <input type="hidden" name="villePOST" value="<?= htmlspecialchars($ville) ?>">
      ...
      while ($l = mysqli_fetch_assoc($result)): ?>
        <section>
          ...
          <div class='reserve-section'>
            ...
            <a class='btn-reserver' href='hotel.php?idHotel=<?= $l['idHotel']?>&nbp=<?= $_POST['personnes'] ?>&depart=<?=$_POST['depart']?>&arrivee=<?=$_POST['arrivee']?>'>Voir l'offre</a>
          </div>
        </section>
    <?php endwhile;
    else:
